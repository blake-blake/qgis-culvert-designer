...
'''
Step 2: Culvert intersections & empty 1d_nwk
- Inputs: road batter toes (vector), DEM, stream network (vector), approx. road width (m)
- Outputs: inlets/outlets (vector), culvert network (vector), pour points (vector)
'''
class Step2_CulvertNetwork(BaseAlgo):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(QgsProcessingParameterVectorLayer(self.P_STREAM, self.tr("Chosen stream network (vector)"), [QgsProcessing.TypeVectorAnyGeometry]))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        stream_vector = self.parameterAsVectorLayer(parameters, self.P_STREAM, context)
        ...
        intersections = find_road_intersections(context, feedback, folders, stream_vector.source(), road_layer)
        nwk = create_culvert_network(context, feedback, folders, dem_layer, intersections, roadw)
        pp = extract_pour_points(context, feedback, folders, nwk)

        produced = {"inlets_outlets": intersections, "culvert_network": nwk, "pour_points": pp}
        write_manifest(base, produced)
        if do_add:
            add_to_project([nwk, pp])
        return produced